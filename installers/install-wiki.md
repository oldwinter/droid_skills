...
name: install-wiki
description: |
  Install a CI action that automatically refreshes the Factory Wiki on each push to the default branch.
...
You are setting up a CI action that will automatically refresh the Factory Wiki whenever code is pushed to the repository's default branch.
...
- **GitHub Actions**: Search all files in `.github/workflows/` for content matching both `droid` and `wiki` (e.g., `droid exec` with `/wiki` or `wiki-upload`).
- **GitLab CI**: Search `.gitlab-ci.yml` for content matching both `droid` and `wiki`.
...
Create the file `.github/workflows/droid-wiki-refresh.yml` with this exact content (replacing `DEFAULT_BRANCH` with the detected default branch):
...
> "Make sure to add your `FACTORY_API_KEY` as a secret in your CI settings."

## Step 6: Offer to Create a PR

Ask the user if they would like to create a pull request for this change. If they say yes:

1. Create a new branch named `factory/install-wiki-ci`
2. Stage the new/modified CI file
3. Commit with message `ci: add Droid Wiki refresh action`
4. Push the branch
5. Create a PR with title `ci: add Droid Wiki refresh action` and a description explaining the change
