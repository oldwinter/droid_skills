...
3. **Wait for readiness** by polling the port with retries:
   ...
   If the server does not become ready within 60 seconds (30 retries \xD7 2s), add warning: `"Screenshots for <app_name> skipped: dev server failed to start on port <port>"` and skip that app. Do not abort the entire phase — continue with other apps.
