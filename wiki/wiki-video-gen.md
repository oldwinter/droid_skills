...
name: wiki-video-gen
description: |
  Generate Factory-branded HyperFrames video overviews for repository wikis.
...
Generate a polished repository overview video from `droid-wiki/` pages and real repository evidence. The output is a Factory-branded MP4 with clear TTS narration, VTT captions, useful architecture visuals, and upload-ready metadata.
...
- Working directory: `<repo>/.factory/video/wiki/<slug>/`
- Canonical upload artifact: `<wikiDir>/video/overview.mp4`
- Canonical caption artifact: `<wikiDir>/video/captions.en.vtt`
- Metadata handoff: `<repo>/.factory/video/wiki/<slug>/videoOverview.json`
...
`wiki-upload` must discover both `<wikiDir>/video/overview.mp4` and `<wikiDir>/video/captions.en.vtt`. The MP4 must contain video and audio only. Captions are delivered by the VTT sidecar and enabled by the Factory wiki player.
...
If a prior video is reused, do not create a new MP4. Write metadata with `status: "ready"`, include the prior run id and diff rationale in `warnings`, and tell the caller to pass `--copy-from-wiki-run-id <priorWikiRunId>` to `droid wiki-upload`.
...
- Keep all temporary tooling under `.factory/video/wiki/<slug>/`.
...
- Anchor all visual styling to the Factory Brand System at `https://factory-brand-guide.vercel.app/`.
- Do not ship a text-only slide deck, repeated centered cards, or generic `FACTORY` text branding.
...
- Do not upload to Factory cloud, commit, push, or clean generated workspaces unless explicitly asked. The caller handles upload.
...
2. `wikiDir`: explicit user/caller value, otherwise `<repoRoot>/droid-wiki`.
...
Use `droid-wiki/` as private research context. The video should describe the **repository**, not the wiki or documentation artifact. Do not say “droid-wiki”, “generated wiki”, “wiki overview”, or “this wiki” unless the repository itself is a documentation product.
...
Apply Factory brand defaults without waiting for restyling prompts. Anchor every visual decision to the Factory Brand System at `https://factory-brand-guide.vercel.app/`. In browsing-capable sessions, fetch or inspect that page before composing. In offline sessions, use the fallback tokens below and write a metadata warning that the live brand guide could not be consulted.
...
- Primary accent: Factory orange `#EE6018`, accent only.
...
- Language: say Factory or Droids, not Factory AI in running text. Avoid "AI-powered", "copilot", "assistant", "helper", "magic", and empty superlatives.
...
Do not modify repo-level manifests. It is acceptable to create a workspace-local `package.json` inside `.factory/video/wiki/<slug>/` when needed.
...
- Start from a Factory shell: black canvas, official white lockup or rotor in the header, top hairline, scene counter, concise repo/context label, and bottom progress indicator.
...
- Do not include a visible `.captions` container, `.caption` nodes, or subtitle text in `index.html`. Captions belong in `assets/captions.en.vtt` and are shown by the Factory wiki player.
- Do not use generic text branding such as a plain orange `FACTORY` word. Use the official lockup/rotor asset, or a fallback mono text label only when assets cannot be fetched.
...
Do not embed a `<video>` tag into `overview/index.md` by default. Factory cloud rendering uses `videoOverview` metadata and the uploaded MP4. Only add a markdown/HTML embed when the user explicitly asks for a local-only wiki embed.
...
- Inspect at least one poster/frame from the visual-only MP4 before muxing. Fail and revise if it looks like a generic text deck, has unstyled subtitle text in the top-left corner, uses orange headlines or large orange fills, lacks a Factory header, or shows a full progress bar at the start.
